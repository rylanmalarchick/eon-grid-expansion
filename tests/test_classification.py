...
def test_hard_requires_strong_control_failure_and_open_gurobi_gap() -> None:
    ...
    assert _classify(mps_negative=True, tree_negative=True, layer_b_mip_gap=0.05) == "hard"
    # Without the open Gurobi gap it is not hard.
    assert _classify(mps_negative=True, tree_negative=True, layer_b_mip_gap=0.0) != "hard"
...
def test_nisq_path_a_candidate_is_the_n20_verdict() -> None:
    ...
    # NISQ-runnable reduced QUBO, with tree_negative False (the reduced QUBO contracts
    # within budget at n=20). This is exactly the instance the pre-Wave-5 gate buried
    # as "ambiguous" by demanding tree_negative on the same n=20 instance.
    assert (
        _classify(
            layer_a_hard=True,
            mps_negative=True,
...
def test_layer_a_hard_without_mps_negative_is_ambiguous() -> None:
    assert _classify(layer_a_hard=True, mps_negative=False) == "ambiguous"


def test_mps_negative_alone_is_ambiguous() -> None:
    # MPS-negative reduced QUBO but an easy Layer A parent and a contractible TN:
    # not a Path-A candidate, not easy, not hard.
